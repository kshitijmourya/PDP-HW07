...
public class FreecellMultiMoveModel extends FreecellModelAbstract {
  ...
  public LinkedList<Cards> multiCardMove(List<LinkedList<Cards>> pilesInput, int pileNumber, int cardValue) {
  ...
  public void move(PileType source, int pileNumber, int cardIndex,
    ...
    if (!hasGameBegun) {
      ...
      Cards card_shifting;
      LinkedList<Cards> multiple_shifts = new LinkedList<>();

      if (source == PileType.CASCADE) {
        List<LinkedList<Cards>> test_piles = new ArrayList<>();
        test_piles.addAll(this.cascadePiles.getPiles());

        multiple_shifts = multiCardMove(test_piles, pileNumber, cardIndex);

        // MUST PUT CARDS INTO DESTINATION PILE ONE BY ONE FROM FIRST SLOT, THEN REMOVE THEM AT
        // THE SAME TIME FROM SOURCE PILE. WILL NEED TO CHANGE REMOVE FUNCTION FOR CASCADE IN
        // THIS CLASS.
